...
int main() {
    ...
    Piece* myBishop = gameBoard->table[1]['c' - 'a' + 1];
    ...
    while (true) {
        ...
        if (strncmp(command, "xboard", strlen("xboard")) == 0) {
            ...
            if (testCounter < 2) {
                ...
                gameBoard->showBoard();
                availablePos = myBishop->findPositions(gameBoard);
                sz = availablePos.size();
                for (i = 0; i < sz; i++) {
                ...
                // cout << "cv1 " << sz << endl;
                i = rand() % sz;
                cout << "move " << myBishop->position.second << 9 - myBishop->position.first << availablePos[i].second << 9 - availablePos[i].first << endl;
                gameBoard->table[myBishop->position.first][myBishop->position.second - 'a' + 1] = NULL;
                gameBoard->table[availablePos[i].first][availablePos[i].second - 'a' + 1] = myBishop;
                myBishop->position.first = availablePos[i].first;
                myBishop->position.second = availablePos[i].second;
                testCounter++;
            }
        }
    }
