...
void removeMove(Piece *&piece, pair<int, char> move, pair<int, char> old_p, int color, Piece *&captured, int moved, int promoted){
...
void apply_move(Piece *&piece, pair<int, char> move, pair<int, char> &old_p, int color, Piece *&captured, int &moved, int &promote){
    moved = 0;
    promote = 0;
    if(piece->name == 'K'){
...
int alphabeta_negamax(int depth, int player, pair<pair<int, char>, Piece*> &best_move, int alpha, int beta) {
    ...
    for (auto move : all_moves) {
        ...
        //gameBoard->showBoard();
        int moved, promoted;
        apply_move(move.second, move.first, old_p, 1 - player, captured, moved, promoted);
        //cout<<endl;
        //gameBoard->showBoard();
        //cout << endl;
        ...
        */}
        ...
        if (alpha >= beta) {
            removeMove(move.second, move.first, old_p, 1 - player, captured, moved, promoted);
            break;
        }
